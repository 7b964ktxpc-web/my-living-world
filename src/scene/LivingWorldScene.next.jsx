...
import { Canvas, Float, Stars, Text, useFrame } from '@react-three/fiber';
...
import { idleWorldMotion } from './worldMotion';
import { createNpcState, tickNpc, npcReaction } from '../game/npcRuntime.js';
import { npcQuestMessage } from '../game/npcQuest.js';
...
function Floor({ world }) {
  return <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -1.35, 0]} receiveShadow><planeGeometry args={[12, 7]} /><meshStandardMaterial color={COLORS[world] || COLORS.space} /></mesh>;
}

function Environment({ world }) {
  if (world === 'space') return <Stars radius={45} depth={30} count={900} factor={1.8} />;
  if (world === 'trains') return <mesh position={[0, -1.15, 0]}><boxGeometry args={[8, 0.06, 0.45]} /><meshStandardMaterial color="#3b3b3b" /></mesh>;
  return null;
}
...
function MovingObject({ object, index, total, action, onSelect }) {
  ...
  useFrame((_, delta) => {
    setMotion((state) => {
      ...
      const steer = Math.min(1, delta * 2.8 + Math.sin(progress * Math.PI) * 0.16);
      return { ...state, x: state.x + (target.x - state.x) * steer, z: state.z + (target.z - state.z) * steer, phase: state.phase + delta };
    });
  });
  return <group position={[motion.x, object.world === 'space' ? motion.altitude || 0 : -0.08, motion.z]} onClick={(event) => { event.stopPropagation(); onSelect(object); }}><LivingObject3D object={object} actionProgress={actionRunning(action) ? actionProgress(action) : 0} actionRunning={actionRunning(action)} /></group>;
}

export default function LivingWorldScene({ world, objects = [], selectedId, onSelect, onPoint, onWorldEvent, action, minigame, collectedObjectives = [], onObjectiveCollect }) {
  const [selected, setSelected] = useState(null);
  const [npcs, setNpcs] = useState(() => createNpcState(world));
  const [npcEvent, setNpcEvent] = useState('idle');
  const [coOp, setCoOp] = useState(() => createCoOpState(world));
  const positions = useRef({});
  const current = useMemo(() => objects.filter((item) => item.world === world).slice(0, 8), [objects, world]);
  const points = useMemo(() => pointsForWorld(world), [world]);
  const guide = guideState(world, collectedObjectives);
  const environment = environmentState(world, coOp.action, true);
  ...
  const emitCoOp = (event, extra = {}) => {
    const next = applyCoOpEvent(coOp, event);
    setCoOp(next);
    if (onWorldEvent) onWorldEvent({ ...extra, coOp: next });
  };

  useFrame((_, delta) => {
    setNpcs((list) => list.map((npc) => {
  ...
  return <Canvas camera={{ position: [0, 1, 6.4], fov: 45 }} shadows onPointerMissed={() => { setSelected(null); if (onSelect) onSelect(null); }}>
    <ambientLight intensity={1.35} />
    <directionalLight position={[4, 6, 4]} intensity={2.1} castShadow />
    <Environment world={world} /><Floor world={world} />
    ...
    {current.map((object, index) => <MovingObject key={object.id || index} object={object} index={index} total={current.length} action={action} onSelect={(item) => { setSelected(item); onSelect?.(item); }} />)}
    {selected && <Float><Text position={[0, 1.8, 0]} fontSize={0.22} color="white" anchorX="center">{selected.label || selected.name}</Text></Float>}
    <NPCQuestBubble world={world} event={npcEvent} visible={Boolean(minigame && npcEvent !== 'idle')} />
    <CoOpStatusBubble world={world} event={coOp.event || 'near'} visible={Boolean(minigame?.status === 'running')} />

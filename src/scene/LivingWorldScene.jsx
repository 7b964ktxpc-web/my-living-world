...
function MovingObject({ object, index, total, action, onSelect }) {
  ...
  useFrame((_, delta) => setMotion((state) => {
    ...
    const target = pointForObject(object.world, object.type, index); if (!target) return { ...state, phase: state.phase + delta };
...
export default function LivingWorldScene({ world, objects = [], selectedId, onSelect, onPoint, onWorldEvent, action, minigame, collectedObjectives = [], onObjectiveCollect }) {
  ...
  useFrame((_, delta) => setNpcs((list) => list.map((npc) => { const targetObject = current.find((item) => item.id === npc.followTarget?.objectId); const target = targetObject ? positions.current[targetObject.id] : npc.followTarget; return tickNpc({ ...npc, followTarget: target }, delta); })));
  ...
  return <Canvas camera={{ position: [0, 1, 6.4], fov: 45 }} shadows onPointerMissed={() => { setSelected(null); onSelect?.(null); }}>
    ...
    <NPCQuestBubble world={world} event={npcEvent} visible={Boolean(minigame && npcEvent !== 'idle')} /><CoOpStatusBubble world={world} event={coOp.event || 'near'} visible={Boolean(minigame?.status === 'running')} /><OrbitControls enablePan={false} minDistance={4} maxDistance={9} />
  </Canvas>;
}

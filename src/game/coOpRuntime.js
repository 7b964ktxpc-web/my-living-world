...
export function applyCoOpEvent(state,event,now=Date.now()){
 ...
 if(event==='success')return {...state,event,team,scene:{world,event,action:'celebrate',kind:'celebrate',emoji:'🎉',worldAction:scene?.action||null},updatedAt:now};
...
export function coOpEventForSuccess(world){return {type:'success',team:teamStateFor(world,'success'),scene:{world,event:'success',action:'celebrate',kind:'celebrate',emoji:'🎉',worldAction:sceneTeamEvent(world,'success')?.action||null}};}

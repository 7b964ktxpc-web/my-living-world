...
export default function TVMode({world='space',objects=[],selectedId=null,action=null,stars=0,onClose,onControl}){
 const [session,setSession]=useState(null);const [connected,setConnected]=useState(false);const [fullscreen,setFullscreen]=useState(false);const [copied,setCopied]=useState(false);const [transport,setTransport]=useState(null);
 useEffect(()=>{const created=createTvSession();setSession(created);const t=createTransport(created.id,{onStatus:s=>setConnected(s==='connected'||s==='local'),onMessage:m=>{if(m.type==='hello'){setConnected(true)}if(m.type==='control')onControl?.(m.payload||{})}});t.send(makeHello('tv'));setTransport(t);return()=>t.close()},[onControl]);
